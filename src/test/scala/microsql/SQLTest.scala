...
class SQLTest extends FeatureSpec with GivenWhenThen {
  ...
  feature("The user can run select queries"){
    scenario("fetching resultset rows as Maps") {
      ...
      val result = executeForResult("select * from student")( extractToMap )
      assert(result.head("ID")("value") == 1)
      assert(result.drop(1).head("NAME")("value") == "marry")
      assert(result.drop(1).head("NAME")("type") == "VARCHAR")
      c.close()
    }
